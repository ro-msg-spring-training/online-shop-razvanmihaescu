...
create table address (id integer not null, city varchar(255), country varchar(255), county varchar(255), street_address varchar(255), primary key (id)) engine=InnoDB;
...
insert into hibernate_sequence values ( 1 );
insert into hibernate_sequence values ( 1 );
create table location (id integer not null, name varchar(255), location_address integer, primary key (id)) engine=InnoDB;
create table order_detail (id integer not null, quantity integer, order_id integer, product_id integer, primary key (id)) engine=InnoDB;
create table orders (id integer not null, created_at datetime, customer_user_id integer, delivery_address_id integer, shipped_from_id integer, primary key (id)) engine=InnoDB;
...
alter table location add constraint FKddyggwi8r4a2jicgsbryncv3k foreign key (location_address) references address (id);
alter table order_detail add constraint FKrws2q0si6oyd6il8gqe2aennc foreign key (order_id) references orders (id);
alter table order_detail add constraint FKb8bg2bkty0oksa3wiq5mp5qnc foreign key (product_id) references product (product_id);
alter table orders add constraint FK8fvgue6769ma18to0aiv8f54s foreign key (customer_user_id) references user (user_id);
alter table orders add constraint FKbwhiubtkxf94knbm9oo55wdbm foreign key (delivery_address_id) references address (id);
